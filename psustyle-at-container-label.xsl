...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
	...
	<!--
        ...
        * EDITOR:           Ben Goldman                             
        *                   goldman@psu.edu                               *
        * DEVELOPER(Aeon):  Maryam Kutchemeshgi                           * 
        *                   mxk128@psu.edu                                *
        *                                                                 *
	...
	*		   Integration with Aeon - Added code to be able  *
	*		   to submit requests to Aeon via EAD 		  *
	*								  *
        * UPDATED          May 31, 2012                                   *
		...
		<html xmlns="http://www.w3.org/1999/xhtml" lang="en" xml:lang="en">
			<head>
				...
				<link rel="stylesheet" type="text/css"
					...
					media="screen"/>
				<link rel="stylesheet" type="text/css"
					href="http://www.libraries.psu.edu/findingaids/css/aeonRequestsDialog.css"
					media="screen"/>

				<script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jquery/2.0.0/jquery.min.js"/>
				<script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jqueryui/1.11.0/jquery-ui.min.js"/>
				<script type="text/javascript" src="https://www.libraries.psu.edu/findingaids/js/aeonRequestDialog.js"/>
				<script type="text/javascript" src="https://www.libraries.psu.edu/findingaids/js/jquery.jqote2.min.js"/>
				<script type="text/javascript" src="https://www.libraries.psu.edu/findingaids/js/fastickynav.js"/>
				<script type="text/javascript" src="https://www.libraries.psu.edu/findingaids/js/aeonSettings.js"/>
				<!-- Aeon end -->
...
<!--
                        //    function doAeonRequest() {
                        //          var x = document.getElementById("requestForm").elements.length;
                        //            alert ("in doAeonRequest " + x);
				...
				<script type="text/javascript" src="/etc/designs/psul/js/gaAddons-2.1.2.min"/>
				<script type="text/javascript">
                    var _gaq = _gaq || [];
                    _gaq.push(
	...
	<xsl:template name="css">
		<!-- <link rel="stylesheet" type="text/css" href="http://ajax.googleapis.com/ajax/libs/jqueryui/1.10.4/themes/redmond/jquery-ui.css" media="screen"/> -->
		<style type="text/css">
			html{
			    margin:0;
			    padding:0;
			...
			#checkbox_toggle .aeon_submit{
			    visibility:visible;
			}
			
			#checkbox_toggle.show .aeon_submit{
			    visibility:visible;
			}
			#checkbox_toggle{
			    padding-top:0px;
	...
	<xsl:template name="header">
		<div id="header">
			<div>
				...
				<!--<xsl:if test="/ead/eadheader/filedesc/publicationstmt/p/extref">-->/ <img
					src="psulogo.gif"/>
				<!--</xsl:if>-->
			</div>
		</div>
	...
	<xsl:template name="toc">
		<div id="toc">
					...
					<dt>
						<a>
							...
							<xsl:choose>
								...
								<xsl:otherwise>Biographical or Historical
			...
			<dl>
				...
				<dt>
					<a href="http://www.libraries.psu.edu/findingaids/finaidaz.html">Browse
						Collections A-Z</a>
				</dt>
				<dt>
		...
		<table width="100%" class="summary">
			...
			<xsl:if test="physloc[string-length(text()|*)!=0]">
				...
				<tr>
					...
					<td>
						<xsl:value-of select="physloc"/>&#160; <xsl:element name="a">
							<xsl:attribute name="target">_blank</xsl:attribute>
							<xsl:attribute name="href"
									><xsl:text>http://cat.libraries.psu.edu/uhtbin/cgisirsi/0/0/0/5?searchdata1=</xsl:text><!--<xsl:for-each select="repository/corpname">
                                    <xsl:choose>
                                        <xsl:when test="subarea = 'Penn State University Archives'">
                                            <xsl:text>PSUA</xsl:text>
                                        </xsl:when>
                                        <xsl:when test="subarea = 'Rare Books and Manuscripts'">
                                            <xsl:text>RBM</xsl:text>
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <xsl:text>HCLA</xsl:text>
                                        </xsl:otherwise>
                                    </xsl:choose>
                                </xsl:for-each>
                                <xsl:text>%20</xsl:text>--><xsl:value-of
									select="unitid"
							/><xsl:text>{099}</xsl:text></xsl:attribute>library
							catalog</xsl:element>. </td>
				</tr>
				<!--</xsl:otherwise>
					</xsl:choose>-->
			</xsl:if>
			<xsl:if test="repository[string-length(text()|*)!=0]">
				...
				<xsl:for-each select="repository">
					...
					<td>
						<!--<xsl:apply-templates select="*"/>-->
						<a href="http://www.libraries.psu.edu/psul/speccolls.html">Special
	...
	<xsl:variable name="requestTitle">
		<xsl:value-of select="/ead/archdesc/did/unittitle"/>
	</xsl:variable>
	<xsl:variable name="requestDate">
		<xsl:value-of select="/ead/archdesc/did/unitdate"/>
	</xsl:variable>
	<xsl:variable name="requestCreator">
		<xsl:value-of select="ead/archdesc/did/origination[@label='creator']/persname"/>
	</xsl:variable>
	<xsl:variable name="requestIdPrefix">
		<xsl:value-of select="translate(ead/archdesc/did/unitid, ' ', '.')"/>
	</xsl:variable>
					...
					<xsl:when test="parent::archdesc">
						<xsl:choose>
							<xsl:when test="self::bibliography">
								<h3>
									<xsl:call-template name="anchor"/>Bibliography</h3>
							</xsl:when>
							<xsl:when test="self::odd">
								<h3>
									<xsl:call-template name="anchor"/>Other Descriptive Data</h3>
							</xsl:when>
							<xsl:when test="self::accruals">
								<h4>
									<xsl:call-template name="anchor"/>Accruals</h4>
							</xsl:when>
							<xsl:when test="self::arrangement">
								<h3>
									<xsl:call-template name="anchor"/>Arrangement of Collection</h3>
							</xsl:when>
							<xsl:when test="self::bioghist">
								<h3>
									<xsl:call-template name="anchor"/>Biographical or Historical
									Information</h3>
							</xsl:when>
							<xsl:when test="self::userestrict">
								<h4>
									<xsl:call-template name="anchor"/>Restrictions on Use</h4>
							</xsl:when>
							<xsl:when test="self::custodhist">
								<h4>
									<xsl:call-template name="anchor"/>Custodial History</h4>
							</xsl:when>
							<xsl:when test="self::altformavail">
								<h4>
									<xsl:call-template name="anchor"/>Alternative Form
									Available</h4>
							</xsl:when>
							<xsl:when test="self::originalsloc">
								<h4>
									<xsl:call-template name="anchor"/>Original Location</h4>
							</xsl:when>
							<xsl:when test="self::fileplan">
								<h3>
									<xsl:call-template name="anchor"/>File Plan</h3>
							</xsl:when>
							<xsl:when test="self::acqinfo">
								<h4>
									<xsl:call-template name="anchor"/>Acquisition Information</h4>
							</xsl:when>
							<xsl:when test="self::otherfindaid">
								<h3>
									<xsl:call-template name="anchor"/>Other Finding Aids</h3>
							</xsl:when>
							...
							<xsl:when test="self::processinfo">
								<h4>
									<xsl:call-template name="anchor"/>Processing Information</h4>
							</xsl:when>
							<xsl:when test="self::relatedmaterial">
								<h4>
									<xsl:call-template name="anchor"/>Related Material</h4>
							</xsl:when>
							<xsl:when test="self::separatedmaterial">
								<h4>
									<xsl:call-template name="anchor"/>Separated Material</h4>
							</xsl:when>
							<xsl:when test="self::appraisal">
								<h4>
									<xsl:call-template name="anchor"/>Appraisal</h4>
							</xsl:when>
						</xsl:choose>
					</xsl:when>
					<xsl:otherwise>
					...
					<xsl:when test="parent::archdesc">
						<xsl:choose>
							<xsl:when test="self::accessrestrict">
								<p>
									<xsl:call-template name="anchor"/>Restrictions on Access</p>
							</xsl:when>
							<xsl:when test="self::scopecontent">
								<p>
									<xsl:call-template name="anchor"/>Scope &amp; Content</p>
							</xsl:when>
						</xsl:choose>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template match="archdesc/dsc">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<h3>
					<xsl:call-template name="anchor"/>Collection Inventory</h3>

				<!-- Aeon begin -->

				<div id="selectionbar">
					<ul class="navbar" id="top_right_nav">
						<!--	 <li id="initsearch"/>-->
						<li id="checkbox_toggle">
							<input type="button" class="aeon_submit" id="aeon_submit"
								value="Submit Request"/>
							<input type="button" class="aeon_cancel" id="aeon_cancel"
								value="Clear Selections"/>
						</li>
						<li id="return_to_top">
							<a href="#c_inventory">Return to Top</a>
						</li>
					</ul>
				</div>

				<p>Click associated checkboxes to select items to request. When you have finished,
					click the Submit Request button.</p>
		...
		<form id="EADRequest" method="post" target="new"
			action="https://aeon.libraries.psu.edu/logon">
			<input name="AeonForm" value="EADRequest" type="hidden"/>
			<input type="hidden" name="Location" value=""/>
			<input type="hidden" name="Site" value=""/>
			<!--	<input name="RequestType" value="Loan" type="hidden"/>-->
			<input name="WebRequestForm" value="GenericRequestManuscript" type="hidden"/>
			<input name="DocumentType" value="Manuscript" type="hidden"/>
			<input type="hidden" name="ItemInfo1" value="{$restrictions}"/>
				...
				<!--<xsl:when test="count(child::*/container/@id) &gt; 1">
					<tr class="{$colorClass}">
						<td colspan="5">
							...
							<table class="parentContainers">
								<xsl:for-each select="child::*/container[@label]">
									...
									<tr class="containerTypes">
										...
										<xsl:for-each select="../container[@parent = $id] | ../container[@id = $id]">
											<td>
												<xsl:value-of select="@type"/>
											</td>
										</xsl:for-each>
							...
							<br class="clear"/>
							<xsl:apply-templates select="*[not(self::did) and not(descendant-or-self::dao) and
		...
		<xsl:choose>
			<xsl:when
				...
				<h4>
					...
					<xsl:variable name="refid" select="../@id"/>
					<xsl:variable name="boxRequestId">
						<xsl:value-of select="concat($requestIdPrefix, $refid, '_box')"/>
					</xsl:variable>

					<div class="visibleRequest">
						<input type="checkbox" name="Request" value="{$boxRequestId}"
							id="Request_{$boxRequestId}"/>
						<input type="hidden" name="ItemTitle_{$boxRequestId}"
							value="{$requestTitle}"/>
						<input type="hidden" name="ItemAuthor_{$boxRequestId}"
							value="{$requestCreator}"/>
						<input type="hidden" name="ItemSubtitle_{$boxRequestId}" value=""/>
						<input type="hidden" name="ItemDate_{$boxRequestId}" value="{$requestDate}"/>
						<input type="hidden" name="CallNumber_{$boxRequestId}"
							value="{$collectionnum}"/>
						<input type="hidden" name="ItemVolume_{$boxRequestId}"
							value="{$levelname} {unitid}"/>
						<input type="hidden" name="ItemIssue_{$boxRequestId}" value=""/>
						<input type="hidden" name="ItemInfo1_{$boxRequestId}" value=""/>
						<input type="hidden" name="ReferenceNumber_{$boxRequestId}" value=""/>
						<label for="Request_{$boxRequestId}">
			...
			<xsl:otherwise>
				<p>
					...
					<xsl:variable name="requestId">
						<xsl:value-of select="concat($requestIdPrefix, $refid)"/>
					</xsl:variable>
					<div class="visibleRequest">
						<input type="checkbox" name="Request" value="{$requestId}"
							id="Request_{$requestId}"/>
						<!--  <input type="checkbox" name="Request" value="{$requestId}" id="Request_{$requestId}">Request This</input> -->
						<input type="hidden" name="ItemTitle_{$requestId}" value="{$requestTitle}"/>
						<input type="hidden" name="ItemAuthor_{$requestId}"
							value="{$requestCreator}"/>
						<input type="hidden" name="ItemSubtitle_{$requestId}" value="{unittitle}"/>
						<input type="hidden" name="ItemDate_{$requestId}" value="{unitdate}"/>
						<input type="hidden" name="CallNumber_{$requestId}" value="{$collectionnum}"/>
						<input type="hidden" name="ItemVolume_{$requestId}"
							value="{container[@type='Box']}"/>
						<input type="hidden" name="ItemIssue_{$requestId}"
							value="Folder {container[@type='Folder']}"/>
						<input type="hidden" name="ItemInfo1_{$requestId}"
							value="{../accessrestrict/p}"/>
						<input type="hidden" name="ReferenceNumber_{$requestId}" value=""/>
						<label for="Request_{$requestId}">
	...
	<xsl:template name="component-did-core">
		...
		<!-- The following adds the contents of physdesc where it exists, right after the date. Mainly for use with PVFs to differentiate prints from negatives.
